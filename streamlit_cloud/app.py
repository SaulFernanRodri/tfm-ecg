"""
Aplicación Streamlit — ECG Diagnosis AI  (Streamlit Community Cloud)

Diferencias respecto a hf/app.py:
  - Los artefactos del modelo se descargan desde HuggingFace Hub en el primer arranque.
  - No requiere tener los ficheros .keras/.joblib en el repositorio GitHub.
...
Despliegue:
  1. Sube esta carpeta a un repositorio GitHub público (o privado con token).
  2. Entra en https://share.streamlit.io → "New app" → selecciona el repo y branch.
  3. Asegúrate de que el fichero principal es app.py.
  4. (Opcional) Añade HF_TOKEN en Secrets para repositorios privados de HF.
"""
...
# ---------------------------------------------------------------------------
# Descarga de artefactos desde HuggingFace Hub
# ---------------------------------------------------------------------------
...
def load_model():
    ...
    model_path = hf_hub_download(
        repo_id=HF_REPO_ID,
        filename="saved_model/v5/best_model.keras",
...
def load_artifacts():
    token = _hf_token()
    _dl = lambda f: hf_hub_download(
        repo_id=HF_REPO_ID, filename=f, repo_type=HF_REPO_TYPE, token=token
    )
...
def load_demo_samples():
    """Carga las muestras de demo (incluidas en el repositorio GitHub)."""
...
def plot_lead_importance_plotly(importances: np.ndarray, class_name: str) -> go.Figure:
    ...
    fig.update_layout(
        height=350,
        title=dict(text=f"Importancia de derivaciones — {class_name}", font=dict(color=TXT)),
        xaxis=dict(title="Caída de puntuación al suprimir derivación",
                   color=TXT, zeroline=True, zerolinecolor="#94a3b8", gridcolor=GRID),
        yaxis=dict(color=TXT),
        paper_bgcolor=BG, plot_bgcolor=BG_PLOT,
        font=dict(color=TXT, size=11),
...
def plot_clinical_influence(influences: np.ndarray, class_name: str) -> go.Figure:
    ...
    fig.update_layout(
        height=240,
        title=dict(text=f"Influencia de variables clínicas — {class_name}", font=dict(color=TXT)),
...
def main():
    ...
    if analyze_btn and st.session_state.get("ecg_ready"):
        ...
        with col:
            ...
            ---
            **Modelo:** ResNet1D-v5 + SE Attention + ASL  
